...
// Locale index (new, mirrors EN index)
...
// Iterate over the union of pages so "red" can occur when locale is missing
const enLikeKeys = new Set(enIndex.keys());
for (const k of locaIndex.keys()) {
  const ek = k.replace(/^files\/[^/]+\//, 'files/en-us/');
  if (!enIndex.has(ek)) enLikeKeys.add(ek);
}

// Generate badges
for (const enKey of enLikeKeys) {
  ...
  // Status:
  // - gray  : page missing in EN
  // - red   : page missing in locale
  // - green : locale newer than EN
  // - yellow: locale present but not newer
  let color;
  ...
  // Derive category/title from whichever side exists
  ...
  // Slug projected to locale space even if missing
  let slug = locaKey
  ...
  console.log('Generated badge:', svgOut);
}
